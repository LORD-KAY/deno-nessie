...
const tableOptions: tableOptions = { dbDialect: "sqlite" };
...
const sqliteStrings = [
  ...
  },
  {
    ...
    solution:
      "CREATE TABLE testTable (testCol int, UNIQUE (testCol));",
  },
  {
    ...
    solution:
      "CREATE TABLE testTable (testCol int, testCol2 int, UNIQUE (testCol, testCol2));",
  },
  {
    ...
    solution:
      "CREATE TABLE testTable (testCol int, PRIMARY KEY (testCol));",
  },
  {
    ...
    solution:
      "CREATE TABLE testTable (testCol int, testCol2 int, PRIMARY KEY (testCol, testCol2));",
  },
  {
  ...
  },
  {
    ...
    solution:
      "CREATE TABLE testTable (created_at timestamptz (0) default current_timestamp, updated_at timestamptz (0) default current_timestamp); DROP TRIGGER IF EXISTS set_timestamp; CREATE TRIGGER set_timestamp BEFORE UPDATE ON testTable FOR EACH ROW BEGIN UPDATE testTable SET updated_at = CURRENT_TIMESTAMP WHERE id=OLD.id\\; END;",
  },
  {
    ...
    solution:
      "CREATE TABLE testTable (updated_at timestamptz (0) default current_timestamp); DROP TRIGGER IF EXISTS set_timestamp; CREATE TRIGGER set_timestamp BEFORE UPDATE ON testTable FOR EACH ROW BEGIN UPDATE testTable SET updated_at = CURRENT_TIMESTAMP WHERE id=OLD.id\\; END",
  },
  {
    name: "Table with text",
    string: (() => {
...
sqliteStrings.forEach(({ name, string, solution }) =>
  Deno.test({
